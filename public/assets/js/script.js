...
(function () {
	...
	if ($('.select-img').length > 0) {
		function formatState (state) {
		  ...
		  var $state = $(
			'<span class="flex flex-shrink-0 items-center justify-start"><img src="' + $(state.element).attr('data-image') + '" class="img-flag rounded-full me-2" / " width="16px"> ' + state.text + '</span>'
		  );
		  return $state;
		};
		$('.select-img').select2({
	...
	if($('.sign-tab-content').length > 0) {
		document.addEventListener('DOMContentLoaded', function () {
			const tabs = document.querySelectorAll('[data-tabs-target]');
			const contents = document.querySelectorAll('.sign-tab-content > div');
			...
			function switchTab(targetId) {
				...
				const targetTab = document.querySelector(`[data-tabs-target="${targetId}"]`);
			...
			radios.forEach(radio => {
				radio.addEventListener('change', () => {
					if (radio.checked) {
						const targetId = `#${radio.id.replace('sign', 'nosign').replace('sign2', 'use-esign')}`; // Map ID to target
						switchTab(targetId);
					}
				});
			});
		
			// Initialize: Show the active tab content on page load
...
function hide() {
    ...
    }, 300); // Match the duration in the `duration-300` class
}*/
/*document.addEventListener('DOMContentLoaded', () => {
	...
	modalButtons.forEach((button) => {
		...
		// Initialize the modal with custom backdrop classes
		if (modalElement) {
			const modalInstance = new Modal(modalElement, {
				backdropClasses: 'z-[9999]', // Custom backdrop z-index
			});

			// Add a click event to show the modal when the button is clicked
	...
	if ($('.collapsearrow').length > 0) {
		$('.collapsearrow').on('click', function() {
			$(this).toggleClass('collapsed'); // Toggle 'collapsed' on the clicked element
			$('body').toggleClass('collapsed'); // Toggle 'collapsed' on the body
		});
	}
