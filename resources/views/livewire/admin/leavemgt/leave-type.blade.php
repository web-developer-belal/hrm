                        ...
                        @foreach ($leaveTypes as $type)
                            <tr class="even:bg-white dark:even-bg-white">
                                ...
                                <td class="px-5 py-2.5 text-gray-900" >
                                    {{ $type->annual_limit }}</td>


                                <td class="px-5 py-2.5 text-gray-500">

                                    <span wire:click="toggleStatus({{ $type->id }})"
                                        class="bg-{{ $type->is_paid == '1' ? 'success' : 'warning' }} text-white rounded text-[10px] font-medium leading-4 py-0.5 px-1.5 inline-flex items-center badge-xs cursor-pointer">
                                        <i
                                            class="ti ti-point-filled me-1"></i>{{ ucfirst($type->is_paid == 0 ? 'unpaid' : 'Paid') }}
                                    </span>
                    ...
                    <form wire:submit.prevent="saveType">
                        <div class="p-4 space-y-4">
                            ...
                            <x-form.input label="Name" name="name" :isRequired="true" :error="true"
                                type="text" placeholder="e.g., Annual Leave, Sick Leave" />

                            <!-- Annual Limit -->
                            <x-form.input label="Annual Limit (Days)" name="annual_limit" :isRequired="true"
                                :error="true" type="number" min="0" max="365" placeholder="e.g., 14" />

                            <!-- Status -->
                            <x-form.select label="Type" name="is_paid" :isRequired="true" :error="true"
