                ...
                <form wire:submit.prevent="saveEmployee" class="space-y-6">
                    <section class="border border-borderColor rounded-[5px] p-4 md:p-5">
                        <div class="mb-4">
                            ...
                            <p class="text-xs text-gray-500 mt-1">Basic profile, organization, and contact information.</p>
                        </div>
                    ...
                    <section class="border border-borderColor rounded-[5px] p-4 md:p-5">
                        <div class="mb-4">
                            ...
                            <p class="text-xs text-gray-500 mt-1">Banking and mobile financial service details for salary disbursement.</p>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                            <x-form.input label="Bank Name" name="bank_name" :error="true" type="text" />

                            <x-form.input label="Account Holder Name" name="account_holder_name"
                                :error="true" type="text" />
                            ...
                            <x-form.input label="Bank Routing Number" name="routing_number"
                                :error="true" type="text" />
                        </div>
                    </section>
                    ...
                    <section class="border border-borderColor rounded-[5px] p-4 md:p-5">
                        <div class="mb-4">
                            ...
                            <p class="text-xs text-gray-500 mt-1">Upload employee documents with compact previews for images and file types.</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                            ...
                            <x-form.file-upload label="Add Offer Letter" name="offer_letter" :error="true"
                                accept=".jpg,.jpeg,.png,.webp,.pdf,.doc,.docx" :oldFiles="$isEditMode && $emp->offer_letter ? [asset(Storage::url($emp->offer_letter))] : []" />
                            <x-form.file-upload label="Add Joining Letter" name="joining_letter" :error="true"
                                accept=".jpg,.jpeg,.png,.webp,.pdf,.doc,.docx" :oldFiles="$isEditMode && $emp->joining_letter ? [asset(Storage::url($emp->joining_letter))] : []" />
                            <x-form.file-upload label="Add Contract & Agreement" name="contract_agreement" :error="true"
                                accept=".jpg,.jpeg,.png,.webp,.pdf,.doc,.docx" :oldFiles="$isEditMode && $emp->contract_agreement ? [asset(Storage::url($emp->contract_agreement))] : []" />
