<div>
    ...
    <div class="md:flex block items-center justify-between page-breadcrumb mb-4">
        <div class="my-auto mb-2">
            <h2 class="mb-1">{{ $isEditMode ? 'Edit Department' : 'Create Department' }}</h2>
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    ...
                    <li class="text-xs text-default">{{ $isEditMode ? 'Edit Department' : 'Create Department' }}</li>
            ...
            <div class="mb-2">
                <a href="{{ route('admin.departments.index') }}"
                    class="flex items-center bg-primary text-sm font-medium py-2 rounded text-white px-3 hover:bg-primary-900 hover:text-white"><i
                        class="ti ti-circle-plus me-2"></i>Departments</a>
            </div>
    ...
    <div class="">
        <div class="card border border-borderColor rounded-[5px] shadow-xs bg-white mb-6">
            <div class="card-header p-5 border-b border-borderColor">
                <h5 class="card-title">{{ $isEditMode ? 'Edit Department' : 'Create Department' }}</h5>
            </div>
            <div class="card-body p-5">
                <form wire:submit.prevent="save" class="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-4">
                    <x-form.input label="Department Name" name="name" :is_required="true" :error="true"
                        placeholder="Enter Department Name" />
                    <x-form.select label="Contact" name="branch_id" :is_required="true" :error="true"
                        :options="$branches" />
